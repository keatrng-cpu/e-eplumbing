(function(){
  ...
  function openChat(){
    ...
    if(!msgs.childElementCount){
      addBubble('Hi — I\'m the Ellingson helper. Ask me about emergencies, booking a call, water heaters, drains, sumps, pricing, or anything else plumbing-related.\n\nIf water is actively flooding or you smell gas, call <a href="'+TEL+'">'+PHONE+'</a> right now — a licensed plumber answers 24/7.', 'bot');
      setChips(starters);
    }
  ...
  function match(q){
    ...
    if(/(human|person|agent|real person|talk to)/.test(q))
      return 'For a real plumber, call or text <a href="'+TEL+'">'+PHONE+'</a> anytime. Or leave details on the <a href="#book">Get a price</a> form.';
    if(/(warranty|guarantee|guarantee|how long)/.test(q))
      return 'We stand behind our work. Ask about the labor warranty and manufacturer coverage on parts when we quote your job.\n\nCall <a href="'+TEL+'">'+PHONE+'</a> or <a href="#book">request service</a>.';
